...
def get_action(state, network: nn.Module, epsilon=0.3):
...
def compute_td_loss(
    ...
    predicted_qvalues_for_actions = predicted_qvalues.squeeze(
        -1
    )  # shape: [batch_size]
    ...
    target_qvalues_for_actions = (
        rewards
        + gamma * predicted_next_qvalues[np.argmax(predicted_next_qvalues)]
    )
